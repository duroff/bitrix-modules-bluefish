<?php
$arAllOptions = Array(
    Array("username", "username", "", Array("text", 50)),
    Array("api_key", "api_key", "", Array("text", 50)),
    Array("url", "Адреса веб-сервиса", "http://bluefish.ru/api/", Array("text", 50)),
);
$aTabs = array(
    ...
    if (strlen($RestoreDefaults) > 0) {
        ...
        foreach ($arAllOptions as $arOption) {
            ...
            if ($arOption[2][0] == "checkbox" && $val != "Y")
                $val = "N";
            COption::SetOptionString("bluefish", $name, $val, $arOption[1]);
        }
    }
...
<?
foreach ($arAllOptions as $arOption):
    $val = COption::GetOptionString("bluefish", $arOption[0], $arOption[2]);
    $type = $arOption[3];
    ?>
        <tr>
